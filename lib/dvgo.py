...
class DirectVoxGO(torch.nn.Module):
        ...
        **kwargs
        ...
        if rgbnet_dim <= 0:
            ...
            self.k0 = torch.nn.Parameter(
                torch.zeros([1, self.k0_dim, *self.world_size])
            )
            self.rgbnet = None
        else:
            ...
            self.register_buffer(
                "posfreq", torch.FloatTensor([(2**i) for i in range(posbase_pe)])
            )
            self.register_buffer(
    ...
    def sample_ray(
        self, rays_o, rays_d, near, far, stepsize, is_train=False, **render_kwargs
    ):
    ...
    def forward(self, rays_o, rays_d, viewdirs, global_step=None, **render_kwargs):
        ...
        if self.pre_act_density:
            ...
            # in-activation
            density = self.grid_sampler(
                rays_pts[~mask_outbbox], F.softplus(self.density + self.act_shift)
            )
...
class MaskCache(nn.Module):
    def __init__(self, path, mask_cache_thres, ks=3):
        ...
        self.mask_cache_thres = mask_cache_thres
        self.register_buffer(
            "xyz_min", torch.FloatTensor(st["MaskCache_kwargs"]["xyz_min"])
        )
        self.register_buffer(
            "xyz_max", torch.FloatTensor(st["MaskCache_kwargs"]["xyz_max"])
        )
        self.register_buffer(
            "density",
            F.max_pool3d(
                st["model_state_dict"]["density"],
                kernel_size=ks,
                padding=ks // 2,
                stride=1,
            ),
        ...
        self.voxel_size_ratio = st["MaskCache_kwargs"]["voxel_size_ratio"].cpu()
    ...
    def forward(self, xyz):
        ...
        if self.nearest:
            density = F.grid_sample(
                self.density, ind_norm, align_corners=True, mode="nearest"
            )
            alpha = 1 - torch.exp(
                -F.softplus(density + self.act_shift) * self.voxel_size_ratio
            )
        elif self.pre_act_density:
            alpha = 1 - torch.exp(
                -F.softplus(self.density + self.act_shift) * self.voxel_size_ratio
            )
            alpha = F.grid_sample(self.density, ind_norm, align_corners=True)
        elif self.in_act_density:
            density = F.grid_sample(
                F.softplus(self.density + self.act_shift), ind_norm, align_corners=True
            )
