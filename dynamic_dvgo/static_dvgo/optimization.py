...
def create_optimizer_or_freeze_model(
    model: DVGO,
    global_step: int,
    lrate_decay: float,
    lrate_density: float,
    ...
    if lrate_density > 0:
        param_groups.append(dict(params=model.density, lr=lrate_density * decay_factor))
    else:
        model.density.requires_grad = False
    
    if lrate_k0 > 0:
        param_groups.append(dict(params=model.k0, lr=lrate_k0 * decay_factor))
    else:
