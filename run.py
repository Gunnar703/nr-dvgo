...
def scene_rep_reconstruction(
    ...
    for global_step in trange(1 + start, 1 + cfg_train.N_iters):
        ...
        if cfg_train.ray_sampler in ["flatten", "in_maskcache"]:
            ...
            sel_b = torch.randint(rgb_tr.shape[0], [cfg_train.N_rand], device="cpu")
            sel_r = torch.randint(rgb_tr.shape[1], [cfg_train.N_rand], device="cpu")
            sel_c = torch.randint(rgb_tr.shape[2], [cfg_train.N_rand], device="cpu")
        ...
        # volume rendering
        render_result = model(
            rays_o, rays_d, viewdirs, global_step=global_step, **render_kwargs
        )
...
if __name__ == "__main__":
    ...
    if args.render_train:
        ...
        imageio.mimwrite(
            os.path.join(testsavedir, "video.disp.mp4"),
